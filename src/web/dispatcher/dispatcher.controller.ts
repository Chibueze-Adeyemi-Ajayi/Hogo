...
export class DispatcherController {
    ...
    @Post("order")
    async addDelivery(@Body() data: CreateDeliveryDTO, @AuthUser() dispatcher: User) {
        return await this.dispatcherService.addDelivery(data, dispatcher)
    }

    @Patch('order/update/:tracking_id')
    @ApiParam({ name: 'tracking_id', type: 'string', description: 'ID of the delivery', required: true })
    async updateDelivery(@Body() data: UpdateDeliveryDTO, @Param("tracking_id") tracking_id, @AuthUser() dispatcher: User) {
        return await this.dispatcherService.updateDelivery(data, tracking_id, dispatcher)
    }

    @Patch("order/cancel/:tracking_id")
    @ApiParam({ name: 'tracking_id', type: 'string', description: 'ID of the delivery', required: true })
    async cancelDelivery(@Body() data: CancelDeliveryDTO, @Param("tracking_id") tracking_id, @AuthUser() dispatcher: User) {
        return await this.dispatcherService.cancelDelivery(data, tracking_id, dispatcher)
    }

    @Get("order/delivery/:tracking_id")
    @ApiParam({ name: 'tracking_id', type: 'string', description: 'ID of the delivery', required: true })
    async viewDelivery(@AuthUser() dispatcher: User, @Param("tracking_id") tracking_id) {
    ...
    async allDelivery(@AuthUser() dispatcher: User, @Query() query: any) {
        return await this.dispatcherService.viewAllDelivery(query, dispatcher);
    }

    // @Post("submit/delivery:tracking_id")
    // @ApiParam({ name: 'tracking_id', type: 'string', description: 'ID of the delivery', required: true })
    // async submitDelivery (@Param("tracking_id") tracking_id: string, @AuthUser() dispatcher: User) {
